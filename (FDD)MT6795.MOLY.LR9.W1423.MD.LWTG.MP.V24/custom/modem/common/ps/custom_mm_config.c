...
/*****************************************************************************
* FUNCTION
*  mm_custom_gmm_hold_auth_req
* DESCRIPTION
*  In old CU SIM card, when CSFB it will authentication fail w/ sync failure first.
*  Pending GMM auth req if there is MM auth is ongoing until HOLD AUTH Timer timeout.
* PARAMETERS
*   none.
* RETURNS
*   KAL_TRUE:  delay GMM auth until HOLD AUTH timer timeout or MM auth finished.
*   KAL_FALSE: follow original design.
*  None
*****************************************************************************/
kal_bool mm_custom_gmm_hold_auth_req(void)
{
#if defined(__GMM_CU_HOLD_AUTH__)    
   return KAL_TRUE;
#else
   return KAL_FALSE;
#endif
