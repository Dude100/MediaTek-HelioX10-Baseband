 ...
 * removed!
 * removed!
 * Basic xboot BL without build error
 ****************************************************************************/
...
volatile brom_rgpd_t brom_multi_rgpd[BROM_MAX_MULTI_UL_PKT_CNT];
...
void brom_reset_multi_rgpd(kal_uint32 chunk_sz, kal_bool is_sdio)
{
	kal_uint32 checksum = 0, chunk_sz_limit = 0, idx = 0;

	memset((void*)(&brom_multi_rgpd), 0 , sizeof(brom_multi_rgpd));
	for (idx = 0 ; idx < BROM_MAX_MULTI_UL_PKT_CNT ; idx ++) {
			brom_multi_rgpd[idx].in_use = KAL_FALSE;
			brom_multi_rgpd[idx].sig = BROM_GPD_SIGNATURE;
		
			/*set tail gpd as BSP GPD with HWO=0*/
			QBM_DES_SET_BPS(&brom_multi_rgpd[idx].bps_tail_gpd);
			checksum = cal_checksum_by_len((kal_uint8 *)(&brom_multi_rgpd[idx].bps_tail_gpd), DEFAULT_QMU_CS_12);
			QBM_DES_SET_CHKSUM(&brom_rgpd.bps_tail_gpd, checksum);
			...
			if (chunk_sz <= chunk_sz_limit) {
				QBM_DES_SET_ALLOW_LEN(&brom_multi_rgpd[idx].data_gpd, chunk_sz);
			} else { 
				/*ERROR*/
				QBM_DES_SET_ALLOW_LEN(&brom_multi_rgpd[idx].data_gpd, chunk_sz_limit); 
			}
			if (idx >= (BROM_MAX_MULTI_UL_PKT_CNT - 1)) { //last gpd
				/*set tail gpd as null bypass gpd*/
...
#ifdef USE_USB_LOCAL_BUF
			QBM_DES_SET_DATAPTR(&brom_multi_rgpd[idx].data_gpd, &brom_multi_rgpd[idx].buf);
#endif
			...
			/*clear HWO to abort current in used GPD*/
			QBM_DES_CLR_HWO(&brom_multi_rgpd[idx].data_gpd);
	}
}
...
void brom_bm_init(kal_uint32 rx_chunk_sz, kal_bool is_sdio)
{
	...
	/*initial multiple rx gpd*/
	brom_reset_multi_rgpd(rx_chunk_sz, is_sdio);
