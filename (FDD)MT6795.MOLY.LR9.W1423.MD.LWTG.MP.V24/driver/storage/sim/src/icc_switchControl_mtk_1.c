...
static kal_bool usim_check_TCK(sim_HW_cb *hw_cb)
{
	...
	//DRV_ICC_print_str("usim_check_TCK");
   ...
   while(SIM_Reg(SIM0_BASE_ADDR_MTK + SIM_COUNT_MTK) == 0 && i++ <= 22)
   {
	...
	if(i >= 23)
	{
		//DRV_ICC_print_str("TCK not present");
	...
	if(ck != 0)
	{
		//DRV_ICC_print_str("TCK checksum err");
		DRV_ICC_print(SIM_PRINT_TCK_CHECKSUM_ERR, 0, 0, 0, 0, 0);
		return KAL_FALSE;
	}

	//DRV_ICC_print_str("TCK checksum ok");
...
static usim_status_enum usim_process_ATR(sim_HW_cb *hw_cb)
{
   ...
   if(ptr[index] == HIST_FIRST_USIM && ptr[index+1] == HIST_SEC_USIM
   	...
   	//DRV_ICC_print_str("historycal byte error");
...
static void usim_process_PTS(sim_HW_cb *hw_cb)
{
	...
	/////DRV_ICC_print_str("usim_process_PTS \r\n");
	if(usim_dcb->app_proto == SIM_PROTOCOL)
	{
		/////DRV_ICC_print_str("PTS not performed (SIM_PROTOCOL) ");
		echoed = KAL_FALSE;
		goto exit;
	}
	if(usim_dcb->reset_mode == USIM_RESET_SPECIFIC)
	{
		/////DRV_ICC_print_str("not performed (specific mode)");
		echoed = KAL_TRUE;
		goto exit;
	}
...
kal_bool usim_select_power(usim_power_enum ExpectVolt, sim_HW_cb *hw_cb)
{
	...
	/////DRV_ICC_print_str("usim_select_power with power: %d \r\n", ExpectVolt);
...
void usim_activation(sim_HW_cb *hw_cb)
{
	...
	/////DRV_ICC_print_str("usim_activation, pow = %d, dir: %d \r\n",usim_dcb->power, usim_dcb->dir);
...
static void usim_deactivation(sim_HW_cb *hw_cb)
{
	...
	/////DRV_ICC_print_str("usim_deactivation\r\n");
...
static void usim_t1end_handler(sim_HW_cb *hw_cb)
{
	...
	switch(usim_dcb->main_state)
	{
		...
		case CMD_RX_STATE:
            ...
            DRV_ICC_print_str("CMD_RX_STATE ");
...
static void usim_hisr_common(sim_HW_cb *hw_cb, kal_uint32 int_status)
{
	...
	PUSH_INT(int_status);
	/////DRV_ICC_print_str("usim_hisr int:%x\r\n",int_status);
#if defined (__SIM_DVT__)
...
static void usim_rx_handler(kal_uint32 int_status, sim_HW_cb *hw_cb)
{
   ...
   kal_uint16 i;
	...
	/////DRV_ICC_print_str("usim_rx_handler ");

	switch(usim_dcb->main_state)
	{
		case ACTIVATION_STATE:
			/////DRV_ICC_print_str("ACTIVATION_STATE ");
			{
	...
	SIM_Reg((SIM0_BASE_ADDR_MTK + SIM_COUNT_MTK));
{
	...
	{
		kal_uint32 i;
		DRV_ICC_print_str("tx:");
		if(USIM_IS_SBLOCK(pcb)&& len ==1)
		{
			DRV_ICC_print_str(" %x",header[T1_INF_INDEX]);
		}
		else
		{
			for(i=0;i<len;i++)
			{
				DRV_ICC_print_str(" %x",adrs[i]);
			}
		}
		DRV_ICC_print_str("\r\n");
	}
...
static kal_bool usim_rx_block_handler(kal_uint32 *adrs, sim_HW_cb *hw_cb)
{
	...
	#if defined(USIM_DEBUG)
	{
		DRV_ICC_print_str("rx:%x %x %x",usim_dcb->header_rx[0],usim_dcb->header_rx[1],usim_dcb->header_rx[2]);
	...
	{	// I-block
		...
		#if defined(USIM_DEBUG)
		{
			...
			for(i=0;i<len;i++)
			{
				DRV_ICC_print_str(" %x",usim_dcb->rx_buf[usim_dcb->rx_index+i]);
			}
			DRV_ICC_print_str("\r\n");
		}
		#endif
...
void sim_baud8N4Test(sim_HW_cb *hw_cb)
{
	...
	sim_PDNDisable_MTK(hw_cb);
		...
		DRV_ICC_print_str("sim_baud8N4Test");
