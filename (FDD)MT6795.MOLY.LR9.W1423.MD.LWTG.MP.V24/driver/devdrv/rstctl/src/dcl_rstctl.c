...
DCL_STATUS DclWDT_Control(DCL_HANDLE handle, DCL_CTRL_CMD cmd, DCL_CTRL_DATA_T *data)
{
	...
	IF_HIFCCCI(volatile kal_uint32 callerAddr;)
	IF_HIFCCCI(volatile kal_uint32 usTime;)
#if defined(MT6290_S00)
    kal_uint16 val;
#endif
	// Check magic number
	...
	IF_HIFCCCI(GET_RETURN_ADDRESS(callerAddr);)
	IF_HIFCCCI(usTime = ust_get_current_time();)
	...
	switch (cmd)
	{
		case WDT_CMD_ENABLE:
			...
			{
			    IF_HIFCCCI(EMM_WriteDbgInfo(EMM_WDT_EN_CALLER, (void*)&callerAddr);)
			    IF_HIFCCCI(EMM_WriteDbgInfo(EMM_WDT_EN_TIME, (void*)&usTime);)
			}
			else
			{
			    IF_HIFCCCI(EMM_WriteDbgInfo(EMM_WDT_DIS_CALLER, (void*)&callerAddr);)
			    IF_HIFCCCI(EMM_WriteDbgInfo(EMM_WDT_DIS_TIME, (void*)&usTime);)
			}
			break;
		...
		#endif /*MT6290_S00*/
		    IF_HIFCCCI(EMM_WriteDbgInfo(EMM_WDT_DRV_RST_CALLER, (void*)&callerAddr);)
			IF_HIFCCCI(EMM_WriteDbgInfo(EMM_WDT_DRV_RST_TIME, (void*)&usTime);)
		...
		case WDT_CMD_ABN_RESET:
		    ...
		    drv_bsi_pmic6339_reg_write(PMIC_6339_ANALDO_CON1_ADDR, val);
		#endif /*MT6290_S00*/
		    IF_HIFCCCI(EMM_WriteDbgInfo(EMM_WDT_ABN_RST_CALLER, (void*)&callerAddr);)
			IF_HIFCCCI(EMM_WriteDbgInfo(EMM_WDT_ABN_RST_TIME, (void*)&usTime);)
		    drv_rstctl_wd_clrCnt(0);
