...
#define NCP6335D            0x10
	...
	{
	        BL_PRINT(LOG_DEBUG,"sts_ncp6335 != NCP6335G\n");
	    ...
	    if(sts_ncp6335 != NCP6335G)
	    {
            ...
            if (sts_ncp6335 == KAL_FALSE && sts_fan53555 == KAL_FALSE)   
            {
                if(ver == 1 && gpio15 == 1)
                {
                    ...
                    BL_PRINT(LOG_DEBUG,"ver == 1 && gpio15 == 1\n");
                    ASSERT(0);
                }
                else
                { 
        ...
        {
             BL_PRINT(LOG_DEBUG,"sts_fan53555 == FAN53555UC08X_PID || sts_fan53555 == FAN53555UC09X_PID\n");
	    ...
	    i2c_pmic_reg_read(NCP6335_SLAVE_ADDR, 0x13, &readback_value);
            ...
            switch(value)
            {
                ...
                case 1:
                    ...
                    i2c_pmic_reg_read(NCP6335_SLAVE_ADDR, 0x10, &readback_value);
                    BL_PRINT(LOG_DEBUG,"ncp6335 write reg[%x] = %x", 0x10, readback_value);
                    i2c_pmic_reg_write(NCP6335_SLAVE_ADDR, 0x16, 0x63);
                    i2c_pmic_reg_read(NCP6335_SLAVE_ADDR, 0x16, &readback_value);
                    BL_PRINT(LOG_DEBUG,"ncp6335 write reg[%x] = %x", 0x16, readback_value);
                    dcl_pmic6339_write_reg(0x65, 0x09); // 1.05V
                    dcl_pmic6339_write_reg(0x63, 0x00); // Shutdown VCORE
                    break;
                case 2:
