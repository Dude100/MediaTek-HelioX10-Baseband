...
#include "L1Trc.h"
	...
	#define RAL_LOG_CR4_TRG_INT(v1) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_1_word(0x0018 | ((short)(v1) << 16));\
	} while(0)
#else
	...
	#define RAL_LOG_MCU_UL_CMPRS_ENTER(v1, v2) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_2_word(0x0518 | ((short)(v1) << 16), (unsigned long)(v2));\
	} while(0)
#else
	...
	#define RAL_LOG_MCU_UL_CMPRS_LEAVE(v1, v2) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_2_word(0x0618 | ((short)(v1) << 16), (unsigned long)(v2));\
	} while(0)
#else
	...
	#define RAL_LOG_MCU_DL_NO_PKT(v1) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_1_word(0x0718 | ((short)(v1) << 16));\
	} while(0)
#else
	...
	#define RAL_LOG_MCU_DL_DECMPRS_ENTER(v1, v2) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_2_word(0x0818 | ((short)(v1) << 16), (unsigned long)(v2));\
	} while(0)
#else
	...
	#define RAL_LOG_MCU_UL_DECMPRS_LEAVE(v1, v2) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_2_word(0x0918 | ((short)(v1) << 16), (unsigned long)(v2));\
	} while(0)
#else
	...
	#define RAL_LOG_MCU_ROHC_PROC_START(v1) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_1_word(0x0A18 | ((short)(v1) << 16));\
	} while(0)
#else
	...
	#define RAL_LOG_MCU_CR4_CTRL_INFO_ADDR(v1, v2) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_2_word(0x0C18 | ((short)(v1) << 16), (unsigned long)(v2));\
	} while(0)
#else
	...
	#define RAL_LOG_MCU_ROHC_ISR(v1) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_1_word(0x0D18 | ((short)(v1) << 16));\
	} while(0)
#else
	...
	#define RAL_LOG_DL_EPDCP_CNT_2_IPID(v1, v2, v3) do {\
		if(RAL_DETAIL_Trace_Filter[0]==1 && (RAL_DETAIL_Trace_Filter[1]&0x04)!=0 )\
			l2trc_send_3_word(0x0E18 | ((short)(v1) << 16), (unsigned long)(v2), (unsigned long)(v3));\
	} while(0)
#else
