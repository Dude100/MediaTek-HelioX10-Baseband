...
typedef struct {
    ...
    kal_bool is_gmm_connected;   //GMM inform SM about the PS Signalling Conn status
    kal_bool is_gmm_attached; // 2012/12/4 mail confirmed. GMM tells SM if GMM is currently attached or detached (GMM owner confirmed that attaching is not possible, only attached or detached is possible)
    kal_bool is_gmm_suspended; //2012/12/4 mail confirmed. GMM tells SM if GMM is currently suspended
    gmmsm_cause_enum suspend_cause; // 2012/12/4 mail confirmed. if GMM is suspended, what is the suspend cause?
